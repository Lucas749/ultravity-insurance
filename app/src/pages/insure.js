...
const [firstTokenName, setFirstTokenName] = useState(null);
const [firstTokenChange, setFirstTokenChange] = useState(null);
const [secondTokenName, setSecondTokenName] = useState(null);
const [secondTokenChange, setSecondTokenChange] = useState(null);
...
const [balanceChanges, setBalanceChanges] = useState(null);
    ...
    async function fetchPremium() {
      ...
      setQuote(result * (coverageAmount*1e18))
      console.log(result)
    }
...
console.log ('premium', quote)
    ...
    const handleBuyNow = async () => {
      ...
      await approveContract.approve("0x08B4434924801AC9C0B1a65D21562f2964b0787B", BigNumber.from('10000000000000000'),  {
        gasLimit: 300000,
      ...
      console.log(quote)
      const premiumAmount =  BigNumber.from(quote.toString());
    ...
    const endpointUrl = `https://ultravity.herokuapp.com/api/insuretx?contract_address=${contractAddress}&chain=5001&cover_amount=${coverageAmount*1e18}&ultravity_score=${ultravityScore}&deviation_threshold=${deviationThreshold}&deadline=${signatureData.deadline}&nonce=${signatureData.nonce}&signature=${signatureData.sig}&balance_changes=${balanceChanges}&transaction=${transactionData}`;
...
useEffect(() => {
  ...
  if (router.query.first_token_name) {
    setFirstTokenName(router.query.first_token_name);
  }
  if (router.query.first_token_change) {
    setFirstTokenChange(parseFloat(router.query.first_token_change));
  }
  if (router.query.second_token_name) {
    setSecondTokenName(router.query.second_token_name);
  }
  if (router.query.second_token_change) {
    setSecondTokenChange(parseFloat(router.query.second_token_change));
  }
  if (router.query.transaction_data) {
      setTransactionData(router.query.transaction_data)
  }
  if (router.query.balanceTransfers) {
    setBalanceChanges(router.query.balanceTransfers)
}
}, [router.query]);
        ...
        <Grid container={true} spacing={2}>
          <Grid item={true} xs={12} md={6}>
              <Card>
                      ...
                      <Typography>Get protection for your transactions</Typography>
                      <br></br>
                      <Table>
                            <TableBody>
                             
                              <TableRow>
                                <TableCell sx={{ fontWeight: 'bold'}}  >Covered Amount (WBIT)</TableCell>
                              ...
                              <TableRow>
                                <TableCell sx={{ fontWeight: 'bold'}}  >Deviation Threshold (%)</TableCell>
                                <TableCell align="right">
                              ...
                              <TableRow>
                                <TableCell sx={{ fontWeight: 'bold'}}  >Premium Payment (WBIT)</TableCell>
                                <TableCell  align="right">
                                  {quote / 1E18}
                                </TableCell>
                              </TableRow>
            ...
            <Grid item={true} xs={12} md={6}>
              <Card>
                      ...
                      <Table>
                            <TableBody>
                              <TableRow>
                                  <TableCell sx={{ fontWeight: 'bold'}} >Covered Address</TableCell>
                                  <TableCell align="right">
                                  {connected ? (
                                  address
                                  ):(
                              ...
                              <TableRow>
                                <TableCell sx={{ fontWeight: 'bold'}}  >Tokens Gained </TableCell>
                                <TableCell  align="right">
                                   {firstTokenChange}  {firstTokenName}
                                </TableCell>
                              </TableRow>
                              <TableRow>
                                <TableCell sx={{ fontWeight: 'bold'}}  >Tokes Spent</TableCell>
                                <TableCell  align="right">
                                {secondTokenChange}  {secondTokenName}
